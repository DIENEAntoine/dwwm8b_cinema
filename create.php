...
// die();
    ...
    if ( $_SERVER['REQUEST_METHOD'] == "POST" ) 
    {
        ...
        if ( isset($post_clean['name']) ) 
        {
            ...
            else if(mb_strlen($post_clean['name'])  > 255 ) // max:255 Caractères
            {
        ...
        if (isset($post_clean['actors'])) {
            ...
            if (empty($post_clean['actors'])) { // Required
                ...
                $create_form_errors['actors'] = "Le nom du ou des acteurs est obligatoirs."; 

            } elseif (mb_strlen($post_clean['actors']) > 255 ) { // max: 255 Caractères
        ...
        if (isset($post_clean['review'])) {
            ...
            } elseif (empty($post_clean['review']) && ($post_clean['review'] != 0) ) {
                
                $create_form_errors['review'] = "La note est obligatoire.";

            } elseif ( !is_numeric($post_clean['review'])) {

                $create_form_errors['review'] = "La note doit etre un nombre.";

            } elseif ( ($post_clean['review'] < 0 ) || ($post_clean['review'] > 5)) {

                $create_form_errors['review'] = "La note doit etre compris entre 0 et 5.";
        ...
        // Arrondir la note a un chiffre apés la virgule
